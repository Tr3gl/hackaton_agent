...
import { formatNumber, getStrings, normalizeLanguage } from "@/lib/i18n";
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-sand-200/50 bg-sand-50/80 px-6 py-4 backdrop-blur-md">
        <div className="flex flex-1 items-center">
          <Link href="/" className="flex items-center gap-2 text-ink-600 hover:text-ink-900 transition-colors">
            ...
            <span className="font-medium text-sm">{strings.backToAgent}</span>
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="grid gap-12 lg:grid-cols-2 items-start">
          ...
          <ProductGallery
            ...
            name={product.name}
            imageUnavailable={strings.imageUnavailable}
            previousLabel={strings.previousImage}
            nextLabel={strings.nextImage}
            thumbnailLabel={strings.thumbnailLabel}
          />
          ...
          <div className="flex flex-col gap-8 lg:sticky lg:top-32">
            ...
            <div className="space-y-4">
              <div className="inline-flex items-center rounded-full bg-sage-100 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-sage-800">
                {product.category || strings.apparelFallback}
              </div>
              ...
              <div className="text-3xl font-light tracking-tight text-ink-700">
                {formatNumber(Number(product.price || 0), language)} TL
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h3 className="font-medium text-ink-900 uppercase tracking-widest text-xs">{strings.productDetails}</h3>
